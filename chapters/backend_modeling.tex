...
As far as this study is concerned, few studies address speaker recognition in co-channel speech signals. 
...
For instance, one might consider the mere presence of two speakers at the same time sufficient to label a segment as overlap, which is an indication of high SIR thresholds ($45dB$ in Fig.~\ref{fig:ch4_swb_overlap_percentage}). 
...
Figure~\ref{fig:ch4_alpha_w_is_1}, shows the case in which $\alpha_w$ is set to $1$ and $\alpha_b$ varies from $0$ to $1$. 
...
In other words, the EER values calculated in Fig.~\ref{fig:ch4_alpha_w_is_1} show performance using clean ($100dB$) PLDA training. 
