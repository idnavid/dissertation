...
Previous chapters focused more on various technical aspects of co-channel speech and less on integrating the developed techniques with other applications. 
This chapter highlights collaborative studies that address co-channel speech analysis in two problems: word-count estimation\footnote{Word-count estimation in co-channel speech is a collaboration with Ali Ziaei, Abhijheet Sangwan, and Prof. John Hansen, a research professor in CRSS. At the time of the study, Ali Ziaei was a fellow graduate student at CRSS, Abhijeet Sangwan was and currently remains a research professor in the department of Electrical Engineering at UT-Dallas.} and in-vehicle speech analysis for driver safety systems\footnote{In-vehicle speech analysis studies were a collaboration with Amardeep Sathyanarayanan, Omid Sadjadi, and Prof. John Hansen. Amardeep and Omid were both PhD students at CRSS during the time these studies were conducted.}. 
...
Each of the studies presented in this chapter have lead to major advancement in developing this dissertation. 

Prior to the study on word-count-estimation, the main overlap detection algorithm developed for this dissertation was Gammatone sub-band frequency modulation (GSFM; see Sect.~\ref{sec:GSFM} in Chapter~\ref{chapter:front-end}). As we have learned, GSFM is highly susceptible to noisy conditions. 
This observation was made when we tried to investigate overlapped speech in Prof-life-log data, a dataset with various types and amounts of noise~\cite{prof-life-log}. 
Pyknograms were the solution to a long-lasting effort of developing a robust overlap detection method for noisy conditions (see Section~\ref{sec:pyknograms} in Chapter~\ref{chap:front-end}). 

In-vehicle speech analysis also contributed to a significant change in perspective in addressing co-channel speech vs. overlap. 
...
It was during this project that the importance of other conversational traits, such as urn-takings and speaker response time, became more visible and prompted further investigation. 
...
The resulting overlap information is embedded into a word-count estimator, which uses a linear minimum mean square estimator (LMMSE) to predict the number of words from the syllable rate. 
...
In cases where the bias factor is used, $S_r(n)$ is replaced by $\big[S_r(n) \hspace{2mm} 1\big]^T$

The linear transformation parameter(s) can be trained using background conversational data that include transcriptions, so that the target word-counts are also available. 
For this study, we rely on a subset of Prof-Life-Log data that has been manually transcribed for word-counts. 
...
SAD reduces false alarms by omitting non-speech regions, which helps avoid detection errors by the syllable detector. 
Spectral subtraction enhances speech regions~\cite{boll1979spectralSubtraction}, allowing the syllable detector to detect voiced regions more accurately. 
...
Figure \ref{fig:wce_system_overview}, shows the proposed WCE system configuration. 
Initially, SAD is performed on the raw data to detect speech locations. 
...
For the curious reader, it suffices to know that performance is measured by statistically modeling driving maneuvers using vehicle dynamic signals (e.g., steering wheel angle, speed, acceleration)~\cite{iv2013amar}. 
...
An experimental setup was conducted that allowed drivers operate the UTDrive vehicle under real-traffic conditions~\cite{utdrive}. 
...
It has also been shown that using sensor data from these portable devices yields to comparable results in maneuver recognition CAN-bus signals obtained from instrumented vehicles~\cite{ITSC, SAE}. 
In this study, data was collected in the UTDrive instrumented vehicle (UTDrive) along with the portable device mounted in the car \cite{ITSC} (See Figure~\ref{fig:our_pic}). However, only the sensor information from the portable device has been used for the entire analysis.
Using a 10.1" Samsung Galaxy Tablet and an Android OS as the portable device platform, an Android application has been developed to collect all the available sensor information on the device synchronously~\cite{ITSC}. 
...
A detailed description of the UTDrive App for portable device and the sensor information can be found in\cite{ITSC}. 
...
An advantage of this experimental setup over previous studies~\cite{ITSC, SAE,ESPA} is that both the in-vehicle speech data and the data required for maneuver recognition are recorded by the portable device. 
...
Statistical information such as maximum lateral acceleration, mean of the vehicle speed, variance of yaw-gyroscope (refer to \cite{ITSC} for a detailed list) form the dominant feature space used in training the maneuver specific models. 
