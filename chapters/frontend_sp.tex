...
This chapter focuses on proposing signal processing techniques to recognize instances of co-channel data where speakers overlap(i.e., overlapped speech detection). 
Single-channel recordings from meetings or conversations are examples during which speakers may overlap. 
In fact, we will see in chapter~\ref{chapter:backend} that there is a direct relation between the number of speakers and the possibility of overlap. 
...
However, over the past decade, due to increased interest in recognition systems such as speaker recognition and diarization, a growing trend of detecting overlapped regions has been observed. 
...
In other words, removing a corrupt (in this case overlapped) speech segment usually does more good then harm in such applications. 
