...
\begin{figure}[h!]
	...
	\caption{\it \small Summary of ILP clustering using CRSSDiar. The binary executables used for each step are shown in the figure. }
...
In addition to PLDA, three other distance measures are implemented for the clustering procedure in CRSSDiar. 
...
Therefore, the cosine metric used in CRSSDiar is $1 - dist_cos(i,j)$ from (\ref{eq:cosine_distance}). 
...
Instead of calculating $\Sigma$ from the segment i-vectors, labeled developments i-vectors are used from an external dataset. 
...
The way CRSSDiar deals with different datasets is very similar to Kaldi. 
In that CRSSDiar binaries are independent of the data-set, but users can create recipe scripts that are specific to a particular corpus design. 
...
CRSSDiar contains labelToRTTM and glpkToRTTM binaries that convert Kaldi vectors to RTTM format. 
...
This section briefly compares CRSSDiar performance under the various distance measures described in Sect.~\ref{sec:plda_ilp}~and~\ref{sec:other_distances_ilp}. 
...
As mentioned throughout this chapter, CRSSDiar is a promising new research platform for speaker diarization. 
...
This chapter provided an outline of CRSSDiar modules implemented so far. 
But as mentioned before, CRSSDiar is a work in progress, both as a research platform and a means of achieving state-of-the-art speaker diarization performance. 
It was pointed out that speech activity detection and overlap detection modules are integrated into CRSSDiar. 
...
Considering the current state of CRSSDiar and the reported error rates, provided sufficient time and effort, state-of-the-art diarization performance is well within our reach.
